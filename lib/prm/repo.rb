...
module Debian
    ...
    def generate_release_gpg(path,release,gpg)
        Dir.chdir("#{path}/dists/#{release}") do
            ...
            if gpg.nil?
              sign_cmd = "gpg -abs --digest-algo \"#{sign_algorithm}\" -u #{gpg} --yes --output Release.gpg -b Release"
            elsif !gpg_passphrase.nil?
              sign_cmd = "echo \'#{gpg_passphrase}\' | gpg -abs --digest-algo \"#{sign_algorithm}\" -u #{gpg} --passphrase-fd 0 --yes -o Release.gpg -b Release"
            else
              sign_cmd = "gpg -abs --digest-algo \"#{sign_algorithm}\" -u #{gpg} --yes --output Release.gpg -b Release"
            end
            system sign_cmd
